        ...
        dict_index_t*	index)
{
	...
	bool	is_supremum = (heap_no == PAGE_HEAP_NO_SUPREMUM);
        lock_rec_print(mode,block,heap_no,index,trx);
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
	...
	case LOCK_REC_SUCCESS_CREATED:
		ret = DB_SUCCESS_LOCKED_REC;
                flag = 1;
                lock_rec_print(mode,block,heap_no,index,thr_get_trx(thr));
		break;
	case LOCK_REC_FAIL:
		ret = lock_rec_lock_slow(impl, mode, block,
					  heap_no, index, thr);
		flag = 1;
		break;
	}
...
int lock_rec_print(ulint mode,const buf_block_t* block,ulint heap_no,dict_index_t* index,const trx_t* trx)
{
 //gaopeng at here test mode
         if(srv_row_lock_detial)
        {
                ...
                if (static_cast<enum lock_mode>(mode & LOCK_MODE_MASK) == LOCK_S)
                        {
                                memcpy(tmp+len,"LOCK_S|",strlen("LOCK_S;"));
                                len += strlen("LOCK_S;");
                        }
                else if(static_cast<enum lock_mode>(mode & LOCK_MODE_MASK) == LOCK_X)
                        {
                                memcpy(tmp,"LOCK_X|",strlen("LOCK_X;"));
                                len += strlen("LOCK_X;");
                        }
                if (mode & LOCK_GAP)
                        {
                            memcpy(tmp+len,"LOCK_GAP|",strlen("LOCK_GAP;"));
                                len += strlen("LOCK_GAP;");
                        }
                else if(mode & LOCK_REC_NOT_GAP)
                        {
                            memcpy(tmp+len,"LOCK_NOT_GAP|",strlen("LOCK_NOT_GAP;"));
                                len += strlen("LOCK_NOT_GAP;");
                        }
		else 
                       {
                            memcpy(tmp+len,"LOCK_NEXT_KEY|",strlen("LOCK_NEXT_KEY|"));
                            len += strlen("LOCK_NEXT_KEY|");
                       }
		     

                if(mode & LOCK_INSERT_INTENTION )
                        {
                             memcpy(tmp+len,"LOCK_INSERT_INTENTION|",strlen("LOCK_INSERT_INTENTION;"));
                                 len += strlen("LOCK_INSERT_INTENTION;");
                        }

                ib::info()<<"TRX ID:("<<trx->id<<") table:"<<index->table_name <<" index:"<< index->name() <<" space_id: "<<block->page.id.space() <<" page_id:"<<block->page.id.page_no() <<" heap_no:"<<heap_no <<" row lock mode:" <<tmp;
	...
	const lock_t*	lock)	/*!< in: record type lock */
{
	...
	fprintf(file, " trx id " TRX_ID_FMT, trx_get_id_for_print(lock->trx));
	        ...
	        if (lock_get_mode(lock) == LOCK_S) {
                fputs(" lock mode S(LOCK_S)", file);
        } else if (lock_get_mode(lock) == LOCK_X) {
                fputs(" lock_mode X(LOCK_X)", file);
        } else {
                ut_error;
        }
  ...
  if(!(lock_rec_get_gap(lock) || lock_rec_get_rec_not_gap(lock)) )
        {
      fputs(" locks gap and rec(LOCK_ORDINARY[next_key_lock])", file);

    }
 //end by gaopeng

        if (lock_rec_get_gap(lock)) {
                fputs(" locks gap before rec(LOCK_GAP)", file);
        }

        if (lock_rec_get_rec_not_gap(lock)) {
                fputs(" locks rec but not gap(LOCK_REC_NOT_GAP)", file);
        }

        if (lock_rec_get_insert_intention(lock)) {
                fputs(" insert intention(LOCK_INSERT_INTENTION)", file);
        }

        if (lock_get_wait(lock)) {
                fputs(" waiting(LOCK_WAIT)", file);
        }
				...
				record */
{
	...
	*inherit = TRUE;
	...
	const lock_t*	wait_for = lock_rec_other_has_conflicting(
				type_mode, block, heap_no, trx,index);
	...
	ulint			heap_no)/*!< in: rec heap number to lock */
{
	...
	if(srv_row_lock_detial)
                {
               ib::info()<<"This TRX help other TRX convert impl lock to expl lock!!!insert often use impl lock!!!!";
	       ib::info()<<"Other TRX:";
               lock_rec_print(type_mode,block,heap_no,index,trx);
	       ib::info()<<"This TRX:";	
                }
