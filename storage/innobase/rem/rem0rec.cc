	...
	const ulint*	offsets)/*!< in: array returned by rec_get_offsets() */
{
	...
	if(srv_row_lock_detial)
		{
			ib::info()<<"PHYSICAL RECORD: n_fields "<<(ulong) rec_offs_n_fields(offsets)<<" compact format; info bits " <<(ulong) rec_get_info_bits(rec, TRUE);
		}

	rec_print_comp_1( rec, offsets);
	rec_validate(rec, offsets);
}
