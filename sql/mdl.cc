...
MDL_context::acquire_lock(MDL_request *mdl_request, ulong lock_wait_timeout)
{
  ...
  */
  ...
  find_deadlock();
   //add by gaopeng
  if (gaopeng_mdl_detail){
  sql_print_information("[Call Acquire_lock] THIS MDL LOCK acquire [WAIT](MDL_LOCK WAIT QUE):");
  my_print_ticket(ticket);}
  //add end
  ...
  */
  ...
  //add by gaopeng
  if (gaopeng_mdl_detail){
  sql_print_information("[Call Acquire_lock] THIS MDL LOCK granted [OK](MDL_LOCK GRANTED QUE):");
  my_print_ticket(mdl_request->ticket);}
  //add end
  return FALSE;
}
...
my_print_ticket(mdl_ticket);

sql_print_information("Get new MDL LOCK [IS]:");
}
 //add end
...
//add by gaopeng 
if (gaopeng_mdl_detail){
sql_print_information("Last MDL LOCK upgrade [TO]:");
my_print_ticket(mdl_ticket);
}
//add end
  DBUG_RETURN(FALSE);
...
//add by gaopeng
   if (gaopeng_mdl_detail){
  sql_print_information("[Call Downgrade_lock]  MDL LOCK downgrade [TO]:");
  my_print_ticket(this);}
//add end
}
...
int my_print_ticket(const MDL_ticket* p_ticket)
{
 THD* my_print_thread = NULL;
  
 my_print_thread = p_ticket->m_ctx->get_thd();

 if(my_print_thread)
{
   sql_print_information("(>MDL PRINT) |Thread id is %u|Current_state: %s| ",my_print_thread->thread_id(),my_print_thread->proc_info);
}
else
{
  ...
  if(p_ticket->m_lock->key.db_name_length())
{
   sql_print_information("(->MDL PRINT) DB_name is:%s ",p_ticket->m_lock->key.db_name());
}
 if(p_ticket->m_lock->key.db_name_length())
{
  sql_print_information("(-->MDL PRINT) OBJ_name is:%s ",p_ticket->m_lock->key.name());
}
 
    switch (p_ticket->m_lock->key.mdl_namespace())
    {
    case MDL_key::GLOBAL:
    ...
    case MDL_key::COMMIT:
      ...
      break;
    case MDL_key::BACKUP:
      sql_print_information("(--->MDL PRINT) Namespace is:%s ","BACKUP");
      break;
    case MDL_key::BINLOG:
      sql_print_information("(--->MDL PRINT) Namespace is:%s ","BINLOG");
      break;
    case MDL_key::TABLESPACE:
  ...
  if(p_ticket->m_is_fast_path)
  {
   sql_print_information("(---->MDL PRINT) Fast path is:(Y)");
   }

  switch (p_ticket->m_type)
{
...
switch ( p_ticket->m_ctx->m_wait.get_status())
{
    case  MDL_wait::EMPTY:
